...
function removeLastFlavor(flavores) {
  flavores.pop()
  return flavores;
}
...
function getFlavorByIndex(orgFlavors, index) {
  let flavor = orgFlavors[index];
  return flavor;
}
...
function removeFlavorByName(originalFlavors, flavor) {
  ...
  for (let i = 0; i < originalFlavors.length; i++) {
    ...
    let originalFlavor = originalFlavors[i];

    // console.log("let originalFlavor = originalFlavors[i]", originalFlavors[i])

    if (originalFlavor === flavor) {
      const index = originalFlavors.indexOf(originalFlavor);
      // console.log("const index = originalFlavors.indexOf(originalFlavor)", index);
      if (index > -1) {
...
function filterByWord(originalFlavors, flavor) {
  ...
  for (let i = 0; i < originalFlavors.length; i++) {
    let orignialFlavor = originalFlavors[i];
    
    if (orignialFlavor.includes(flavor)) {
      newFlavors.push(orignialFlavor);
    }
  }
